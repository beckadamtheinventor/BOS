...
_nomatch:
	...
	call	bos._CpHLDE			; have we reached the end of the table?
	push	af
	ex	de,hl
...
_resloveentrypointsloop:
	...
	call	bos._LoadDEInd_s		; de=offest in lib for function
	ld	hl,(ramlocation)
...
_relocateabsolutesloop:
	ld	de,(endrelocationtbl)
	call	bos._CpHLDE			; have we reached the end of the relocation table
...
_nosetstart:
	...
	call	bos._CpHLDE			; make sure we are done parsing the dependency queue
